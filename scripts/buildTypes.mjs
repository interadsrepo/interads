...
function rewriteImportPath(importPath) {
  ...
  if (importPath.startsWith(coreSrcPath)) {
    return importPath.replace(coreSrcPath, '@interads/ui');
  }

  const stylesSrcPath = path.posix.join('..', 'hook', 'src');
  if (importPath.startsWith(stylesSrcPath)) {
    return importPath.replace(stylesSrcPath, '@interads/hook');
  }

  const systemSrcPath = path.posix.join('..', 'util', 'src');
  if (importPath.startsWith(systemSrcPath)) {
    return importPath.replace(systemSrcPath, '@interads/util');
  }
...
async function main() {
  ...
  const publishDir = path.join(packageRoot, 'lib');
